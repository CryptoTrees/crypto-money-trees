...
contract Trees is Admin {
  ...
  address payable public defaultTreesOwner = msg.sender;
  ...
  function waterTree(uint256 _treeId) public {
    ...
    totalTreePower += 1;
    emit LogWaterTree(_treeId, msg.sender, now);
  }
  ...
  function checkTreesWatered(uint256[] memory _treeIds) public view returns(bool[] memory results) {
    results = new bool[](_treeIds.length);
    uint256 timeSinceLastWater;
  ...
  function checkRewards(uint256[] memory _treeIds) public view returns(uint256[] memory results) {
    uint256 amountInTreasuryToDistribute = address(this).balance / 10;
